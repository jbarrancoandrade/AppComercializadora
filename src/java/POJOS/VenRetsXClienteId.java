package POJOS;
// Generated 19-sep-2014 10:35:39 by Hibernate Tools 3.6.0



/**
 * VenRetsXClienteId generated by hbm2java
 */
public class VenRetsXClienteId  implements java.io.Serializable {


     private String cedula;
     private String codCon;

    public VenRetsXClienteId() {
    }

    public VenRetsXClienteId(String cedula, String codCon) {
       this.cedula = cedula;
       this.codCon = codCon;
    }
   
    public String getCedula() {
        return this.cedula;
    }
    
    public void setCedula(String cedula) {
        this.cedula = cedula;
    }
    public String getCodCon() {
        return this.codCon;
    }
    
    public void setCodCon(String codCon) {
        this.codCon = codCon;
    }


   public boolean equals(Object other) {
         if ( (this == other ) ) return true;
		 if ( (other == null ) ) return false;
		 if ( !(other instanceof VenRetsXClienteId) ) return false;
		 VenRetsXClienteId castOther = ( VenRetsXClienteId ) other; 
         
		 return ( (this.getCedula()==castOther.getCedula()) || ( this.getCedula()!=null && castOther.getCedula()!=null && this.getCedula().equals(castOther.getCedula()) ) )
 && ( (this.getCodCon()==castOther.getCodCon()) || ( this.getCodCon()!=null && castOther.getCodCon()!=null && this.getCodCon().equals(castOther.getCodCon()) ) );
   }
   
   public int hashCode() {
         int result = 17;
         
         result = 37 * result + ( getCedula() == null ? 0 : this.getCedula().hashCode() );
         result = 37 * result + ( getCodCon() == null ? 0 : this.getCodCon().hashCode() );
         return result;
   }   


}


